...
export function getFloatCoord(el: HTMLElement, coord: Coord, mode: FloatMode): Coord;
...
function getFloatCoord1(el: HTMLElement, coord: Coord, mode: FloatMode): Coord {
...
function getFloatCoord2(
    ...
    const x = Math.min(
        coord.x - offsetX,
        window.innerWidth - scrollbarWidth() - Math.ceil(width), // 修正误差
    );
    const y =
        Math.ceil(coord.y + height) > window.innerHeight ? coord.y - height : coord.y;
